...
<body>
  ...
  const config = useMemo(() => ({
    ...
    buttons: [
      ...
      'ul', 'ol', 'todo', '|', // Todo lists (PRO)
      'outdent', 'indent', '|',
      'font', 'fontsize', 'brush', 'paragraph', '|',
      'image', 'video', 'table', 'link', '|',
      'align', 'undo', 'redo', '|',
      'hr', 'eraser', 'copyformat', '|',
      'print', '|', // Emoji picker (PRO)
    ...
    controls: {
      ...
      fontsize: {
        list: ['8', '9', '10', '11', '12', '13', '14', '16', '18', '20', '22', '24', '26', '28', '32', '36', '48', '72']
      },
    ...
    <>
      <Paper elevation={3} style={{ padding: '25px', marginTop: '20px', borderRadius: '12px' }}>
        ...
        <Box mb={3}>
          ...
          >
            <Tab label="Editor" icon={<FormatPaintIcon />} iconPosition="start" />
            <Tab label="Variables" icon={<ContentCopyIcon />} iconPosition="start" />
            <Tab label="PRO Features" icon={<SettingsIcon />} iconPosition="start" />
            <Tab label="AI & Tools" icon={<SmartToyIcon />} iconPosition="start" />
          </Tabs> */}
      ...
      >
        ...
        {templates?.length > 0 ? (
          templates.map((option) => (
            <MenuItem key={option._id} value={option.htmlContent}>
              {option.templateName}
            </MenuItem>
          ))
        ) : (
          <MenuItem value="" disabled>
            No Templates Available
                  ...
                  {/* <Grid item xs={12} sm={6} md={3}>
                    ...
                    >
                      {partners?.length > 0 ? (
                        partners.map((option) => (
                          <MenuItem key={option._id} value={option._id}>
                            {option.partner?.fullName || option.name}
                          </MenuItem>
                        ))
                      ) : (
                        <MenuItem value="" disabled>
              ...
              <Box display="flex" gap={2} justifyContent="flex-end" mt={3} flexWrap="wrap">
                <Button onClick={() => router.push('/templates/pdflists')} color="primary"  variant="outlined" size='large'  style={{marginRight: "10px" }}>Back</Button>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Box sx={{ mb: 3 }}>
                    {[
                      ...
                      >
                        <Box sx={{ color: feature.enabled ? '#1976d2' : '#ff9800' }}>
                          {feature.icon}
                        </Box>
                        <Box sx={{ flex: 1 }}>
