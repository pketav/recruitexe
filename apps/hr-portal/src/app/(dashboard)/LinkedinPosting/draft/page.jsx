    ...
    try {
      ...
      if (result.data.status && Array.isArray(result.data.items)) {
        const mappedDrafts = result.data.items.map((draft) => ({
          ...
          LinkedinAccount: draft.linkedinAccountNames,
    ...
    if (!searchQuery.trim()) {
      ...
      const filtered = drafts.filter(
        (draft) =>
          ...
          draft.linkedinAccountNames.map((e) => e.toLowerCase().includes(searchQuery.toLowerCase())),
      )
            ...
            <Fade in timeout={300}>
              <StyledCard>
                ...
                <CardContent sx={{ pt: 0 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Linkedin Account : {draft.LinkedinAccount} 
                  </Typography>
