...
const PDFTemplatesPage = () => {
  ...
  const handleSaveTemplate = async templateData => {
    try {
      const { templateName, content, selectedProduct: productId, variablesType } = templateData
  ...
  const handleUpdateTemplate = async templateData => {
    try {
      const { templateName, content, templateId, variablesType } = templateData
      if (!templateName.trim()) {
        throw new Error('Template name is required')
      }
  ...
  const getAllPdfList = async () => {
    try {
      setIsLoading(true)
      const response = await axios.get(`${baseUrl}/v1/api/templete/listTemplates`, {
        headers: {
        ...
        >
          ...
          >
            <Box
              ...
              dangerouslySetInnerHTML={{ __html: beautifyTemplate(previewContent) }}
            />
          </DialogContent>
